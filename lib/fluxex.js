...
/** @lends Fluxex# */
{
    ...
    /**
     * Create a promise with provided list
     * @method
     * @param {Array} promises - An array of values or promises
     * @return {Promise} A new promise
     */
    allPromise: function (promises) {
        return withPromise.all(promises, this);
    },
    ...
    executeActions: function () {
        ...
        for (I = 0; I < args.length; I++) {
            rets.push(this.executeAction.apply(this, args[I]));
        }

        return this.allPromise(rets);
    },
     ...
     * @return {Promise} A promise instance
     */
    dispatchs: function (actions) {
        var I, ret = [];

        if (!actions || !actions.length) {
            throw new Error('.dispatchs() require an array param!');
        }

        if (actions.length % 2) {
            throw new Error('.dispatchs() require a param as [action, payload, action, payload ...]!');
       
        }

        for (I=0;I<actions.length;I+=2) {
            ret.push(this.dispatch(actions[I], actions[I+1]));
        }

        return this.allPromise(ret);
    },
    ...
    dispatch: function (name, payload) {
        ...
        return this.createPromise(function (resolve) {
            ...
            resolve(this.allPromise(promises));
        });
    }
});
