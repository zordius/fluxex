...
/** @lends FluxexObject# */
{
     ...
     */
    toString: function () {
        return JSON.stringify(this._context, function (K, V) {
            if (V === undefined) {
               console.warn('[Fluxex] undefined detected on key `' + K + '`! Please fix your store implementation to prevent store undefined into it, or you will see React client side error: `Invariant Violation: You\'re trying to render a component to the document using server rendering but the checksum was invalid.`');
            }
            return V;
        });
    }
